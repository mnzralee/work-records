...
The `previewFeatures = ["prismaSchemaFolder"]` enables multi-file schemas. Without this, Prisma expects everything in one file. Note: in Prisma 6+ (released late 2025), multi-file schemas became GA and you can remove this preview flag. If you're on Prisma 5.x, you still need it.
...
`Error.captureStackTrace` cleans up the stack trace so it starts from where the error was thrown, not from inside the DomainError constructor. Small detail, but it makes debugging significantly easier. Note: this is a V8/Node.js API — it works in Node.js and Chrome, which is what we're targeting. If you ever port this code to a non-V8 runtime (like Bun or Deno), guard this call with `if (Error.captureStackTrace)`.
...
This directive is important. Next.js 14 defaults to Server Components. Providers that use React context (like TanStack Query) must be Client Components. The `'use client'` directive marks this boundary.
...
After you run the seed script (Chapter 3), it will print the list ID. Copy that ID and paste it here. Later, when we add authentication (Chapter 12), we'll pull the list ID from the user's session automatically.
...
Axios for making HTTP requests from the server side. This file runs on the Next.js server, not in the browser.
...
`NextResponse` is Next.js's response helper for API routes.
...
The GET handler. It extracts `listId` from the query string, validates it, calls the backend, and returns the response. If the backend returns an error, we forward the status code and message.

This is the BFF's job: take a frontend request, add any server-side concerns (auth headers, validation), forward to the backend, and return the response. The browser never knows the backend's address, port, or API format.
...
Dynamic route parameter: `[itemId]` in the folder name becomes `params.itemId` in the handler. Next.js extracts it from the URL automatically. In Next.js 15+, `params` is a Promise — you must `await` it before accessing the values.
...
Same pattern. Extract item ID from URL, forward request to backend, return response.

All three BFF routes follow the same structure:
1. Parse the incoming request
2. Validate minimal requirements
3. Forward to the correct backend endpoint
4. Return the backend's response (or a formatted error)

When we add authentication in Chapter 12, each of these routes will also check the user's session and add a JWT header to the backend request. The structure stays the same — we just add one step at the beginning.
...
Update the BFF grocery routes to include the token. Add this helper to `src/lib/backendClient.ts`:
...
Then in each BFF route, add the auth headers:

```typescript
// Example update for GET /api/grocery/items
...
**Principle check:** *Read the error, trust the error.* When auth fails, our system tells you exactly what happened: `INVALID_CREDENTIALS`, `TOKEN_EXPIRED`, `INSUFFICIENT_PERMISSIONS`. Each error has a code, message, and details. When you see a 403, you know the user is authenticated but lacks the required role. When you see a 401, you know the credentials are wrong. Trust these errors — they were designed to guide you.
...
The production stage starts fresh. No TypeScript source, no dev dependencies, no build tools. Just the compiled JavaScript and production `node_modules`. This keeps the image small and secure.
